...
*/
function isUncachedRequest(params) {
...
*/
function getUncachedRateLimitTier(perPage) {
	if (perPage <= PER_PAGE_THRESHOLDS.SMALL) return UNCACHED_RATE_LIMITS.SINGLE_OR_SMALL;
	else if (perPage <= PER_PAGE_THRESHOLDS.MEDIUM) return UNCACHED_RATE_LIMITS.MEDIUM;
	else if (perPage <= PER_PAGE_THRESHOLDS.LARGE) return UNCACHED_RATE_LIMITS.LARGE;
	else return UNCACHED_RATE_LIMITS.VERY_LARGE;
}
/**
...
function determineRateLimit(url, params, config = {}, defaultRateLimit) {
	...
	if (params && !isUncachedRequest(params)) return MAX_RATE_LIMIT;
	if (defaultRateLimit !== void 0) return defaultRateLimit;
	if (isSingleStoryRequest(url, params)) return UNCACHED_RATE_LIMITS.SINGLE_OR_SMALL;
